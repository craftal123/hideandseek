...
public final class HnsCommand implements CommandExecutor, TabCompleter {
    ...
    private void start(CommandSender sender, String[] args) {
        ...
        sender.sendMessage(ChatColor.GREEN + "Started game from transcript format.");
    }
    ...
    private void ask(CommandSender sender, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage(ChatColor.RED + "Usage: /hns ask <questionId>");
            return;
        }
        sender.sendMessage(game.askQuestion(p, args[1]));
    }
    ...
    private String help() {
        return ChatColor.AQUA + "/hns start <hider> <huntersCsv> [seconds], /hns ask <id>, /hns complete, /hns disguise <block>, /hns status, /hns found <hider>, /hns stop";
    }

    @Override
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            return List.of("start", "stop", "ask", "complete", "disguise", "status", "found");
        }
